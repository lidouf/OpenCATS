...
class candidatesListByViewDataGrid extends CandidatesDataGrid
{
    ...
    public function getInnerActionArea()
    {
        ...
        $html .= $this->getInnerActionAreaItemPopup(_('Add To List'), CATSUtility::getIndexName().'?m=lists&amp;a=addToListFromDatagridModal&amp;dataItemType='.DATA_ITEM_CANDIDATE, 450, 350);
        
        if($_SESSION['CATS']->getAccessLevel('pipelines.addToPipeline') >= ACCESS_LEVEL_EDIT) 
        {
            $html .= $this->getInnerActionAreaItemPopup(_('Add To Pipeline'), CATSUtility::getIndexName().'?m=candidates&amp;a=considerForJobSearch', 750, 460);
        }
        
        if(MAIL_MAILER != 0 && $_SESSION['CATS']->getAccessLevel('candidates.canEmail') >= ACCESS_LEVEL_SA)
        {
            $html .= $this->getInnerActionAreaItem(_('Send E-Mail'), CATSUtility::getIndexName().'?m=candidates&amp;a=emailCandidates');
        }
        $html .= $this->getInnerActionAreaItem(_('Export'), CATSUtility::getIndexName().'?m=export&amp;a=exportByDataGrid');
...
class candidatesSavedListByViewDataGrid extends CandidatesDataGrid
{
    ...
    public function getInnerActionArea()
    {
        ...
        $html .= $this->getInnerActionAreaItem(_('Remove From This List'), CATSUtility::getIndexName().'?m=lists&amp;a=removeFromListDatagrid&amp;dataItemType='.DATA_ITEM_CANDIDATE.'&amp;savedListID='.$this->getMiscArgument(), false);
        $html .= $this->getInnerActionAreaItemPopup(_('Add To Pipeline'), CATSUtility::getIndexName().'?m=candidates&amp;a=considerForJobSearch', 750, 460);
